const item = [
  ...
  function displayItems() {
    ...
    console.log(filter);  
   let containerElement = document.querySelector('.container .row');
    let containerData = '';
  item.forEach(i => {
    if(i.title.toLowerCase().includes(filter.toLowerCase())){
      containerData += 
   ...
   <div class="card">
        ...
        <div class="card-body">
          ...
          <p class="card-text"><small class="text-muted">Category: ${i.category}</small></p>
        </div>
        </div>
      </div>`;
    }
  });
  containerElement.innerHTML=containerData;
